...
document.addEventListener('DOMContentLoaded', function() {
    const isLogged = sessionStorage.getItem('adminLoggedIn');
    const savedPin = sessionStorage.getItem('adminPin');

    if (isLogged === 'true' && savedPin) {
        // Jika sudah login, sembunyikan kotak login dan muat data
        document.getElementById('loginOverlay').style.display = 'none';
        loadData();
    } else {
        // Jika belum, pastikan overlay login muncul
...
// 2. FUNGSI PROSES LOGIN DARI OVERLAY
async function prosesLogin() {
    ...
    try {
        ...
        if (data.status === 'success') {
            // PIN Benar: Simpan sesi dan PIN ke browser
            document.getElementById('loginOverlay').style.display = 'none';
            sessionStorage.setItem('adminLoggedIn', 'true');
            sessionStorage.setItem('adminPin', inputPin); // <--- Kunci agar aksi admin tidak ditolak
...
// 3. FUNGSI FETCH AMAN UNTUK KIRIM AKSI ADMIN
async function secureFetch(url, payload) {
    const adminPin = sessionStorage.getItem('adminPin') || "";
    
    // Pastikan PIN selalu ikut terkirim di dalam payload
    ...
    try {
        ...
        try {
            ...
            throw new Error(e.message || "Gagal membaca balasan dari server Google.");
        }
    } catch (error) {
        console.error("Fetch Error:", error);
        throw new Error(error.message || "Koneksi digagalkan oleh browser.");
    }
}
...
async function loadData() {
    ...
    tableBody.innerHTML = '<tr><td colspan="7" class="text-center py-12 text-gray-400 font-medium">Memuat data dari server...</td></tr>';
    ...
    try {
        ...
        tableBody.innerHTML = `<tr><td colspan="7" class="text-center py-12 text-rose-500 font-bold">Gagal memuat: ${error.message}</td></tr>`; 
    }
}
...
async function prosesConfirmAction(event) {
    ...
    try {
        ...
        let result = await secureFetch(WEB_APP_URL, Object.assign({ action: actionCode }, requestBody));
        tutupConfirmModal();
...
function prosesLogout() {
    sessionStorage.clear(); // Bersihkan semua sesi login
    location.reload();      // Muat ulang halaman untuk memunculkan kotak login kembali
}
